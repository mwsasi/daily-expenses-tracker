    ...
    <form id="transaction-form" onSubmit={handleSubmit} className={`bg-white p-5 md:p-6 rounded-2xl shadow-sm border ${isEditing ? 'border-emerald-500 ring-4 ring-emerald-50' : 'border-slate-100'} space-y-4 transition-all`}>
      <div className="flex items-center justify-between mb-1">
        <h3 className="text-base md:text-lg font-black text-slate-800 lowercase">
          {isEditing ? 'edit transaction' : (filterType === TransactionType.INCOME ? 'add income' : 'add expense')}
        </h3>
        {isEditing && (
      ...
      <div className="flex gap-2">
        <button type="submit" className={`flex-grow ${isEditing ? 'bg-emerald-600' : (filterType === TransactionType.INCOME ? 'bg-emerald-600' : 'bg-slate-900')} text-white font-black py-3 rounded-xl shadow-lg transition-all active:scale-95 lowercase text-sm`}>
          {isEditing ? 'update entry' : `add ${filterType === TransactionType.INCOME ? 'income' : 'expense'}`}
        </button>
        {isEditing && (
