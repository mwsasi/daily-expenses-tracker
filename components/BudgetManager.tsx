  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h2 className="text-xl font-black text-slate-800 lowercase">monthly budget settings</h2>
          <p className="text-sm text-slate-500 font-bold lowercase">set monthly limits to keep your expenses in check.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
        {expenseCategories.map(cat => {
          ...
          return (
            <div key={cat.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-50 hover:border-slate-200 transition-all group active:scale-95">
              <div className="flex items-center gap-3">
                <div className={`${cat.color} p-2 rounded-lg text-white shadow-sm`}>
                  <IconComp className="w-5 h-5" />
                </div>
                <span className="font-black text-slate-700 lowercase">{cat.name}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-slate-400 text-[10px] font-black uppercase">RS</span>
                <input 
                  type="number" 
                  placeholder="0.00"
                  value={budgets[cat.name] || ''}
                  onChange={(e) => onUpdate(cat.name, parseFloat(e.target.value) || 0)}
                  className="w-24 px-3 py-1.5 bg-slate-50 border-0 focus:ring-2 focus:ring-emerald-500 rounded-lg text-right font-black text-slate-800"
                />
              </div>
            </div>
          );
