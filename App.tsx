...
import { Trash2, History, Wallet, LayoutDashboard, PlusCircle, FileText, Calendar, ArrowRightCircle, CheckCircle2, TrendingUp, Plus } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const addTransaction = (newTransaction: Transaction) => {
    setTransactions(prev => {
      // Check if a transaction for the same date and category already exists
      ...
      if (existingIndex !== -1) {
        // Found a match, let's aggregate (cumulative)
        const updated = [...prev];
  ...
  const summary = useMemo<DailySummary & { todayExpenses: number, todayIncome: number }>(() => {
    ...
    return {
      ...
      netSavings: savings,
      currentBalance: opening + income - expenses,
      todayExpenses: todayExp,
      todayIncome: todayInc
    };
  }, [transactions, todayStr]);

  // Group transactions by date for grouped display
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 hidden md:block">
          <nav className="flex gap-8">
            ...
            <button
              ...
              className={`py-4 px-1 border-b-2 font-medium text-sm transition-all ${
                activeTab === 'income' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-500'
              }`}
            >
              <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`py-4 px-1 border-b-2 font-medium text-sm transition-all ${
                activeTab === 'manage' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-500'
              }`}
            >
              <div className="flex items-center gap-2">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {summary.openingBalance === 0 ? (
                <div className="bg-blue-600 rounded-2xl p-6 text-white shadow-xl shadow-blue-200 flex flex-col justify-between overflow-hidden relative min-h-[160px]">
                  ...
                  <div className="flex gap-2 relative z-10">
                    <input 
                      ...
                      className="w-full px-4 py-2 rounded-xl border-0 text-slate-900 focus:ring-2 focus:ring-blue-400 font-bold text-sm"
                    />
                    <button 
                      onClick={setInitialOpeningBalance}
                      className="bg-white text-blue-600 px-4 py-2 rounded-xl font-bold hover:bg-blue-50 transition-all text-sm whitespace-nowrap"
                    >
                      Save
                    </button>
                  </div>
                ...
                <div className="bg-white border border-blue-100 rounded-2xl p-6 flex flex-col justify-between shadow-sm min-h-[160px]">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      onClick={() => {
                        const existing = transactions.find(t => t.category === Category.OPENING_BALANCE);
                        if (existing) {
                          setOpeningInput(existing.amount.toString());
                          removeTransaction(existing.id);
                        }
                      }}
                      className="text-[10px] font-bold text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition-all border border-blue-100 uppercase"
                    >
                      Adjust Balance
                    </button>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Current Opening Balance</span>
                    <p className="text-2xl font-black text-slate-900">Rs {summary.openingBalance.toFixed(2)}</p>
                  </div>
                </div>
              )}

              {/* Quick Daily Income Section */}
              <div className="bg-emerald-600 rounded-2xl p-6 text-white shadow-xl shadow-emerald-200 flex flex-col justify-between overflow-hidden relative min-h-[160px]">
                ...
                <div className="flex gap-2 relative z-10">
                  <input 
                    type="number"
                    placeholder="Enter Today's Earning"
                    value={quickIncomeInput}
                    onChange={(e) => setQuickIncomeInput(e.target.value)}
                    className="w-full px-4 py-2 rounded-xl border-0 text-slate-900 focus:ring-2 focus:ring-emerald-400 font-bold text-sm"
                  />
                  <button 
                    onClick={addQuickDailyIncome}
                    className="bg-white text-emerald-600 px-4 py-2 rounded-xl font-bold hover:bg-emerald-50 transition-all text-sm whitespace-nowrap"
                  >
                    Add Income
                  </button>
                </div>
            ...
            <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-emerald-900">Today's Cumulative Progress</h3>
                  <p className="text-sm text-emerald-700">Totals for {new Date().toLocaleDateString(undefined, { weekday: 'long', day: 'numeric', month: 'short' })}</p>
                </div>
              </div>
              <div className="flex gap-8">
                <div className="text-center">
                  <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Today's Income</p>
                  <p className="text-xl font-black text-emerald-900">Rs {summary.todayIncome.toFixed(2)}</p>
                </div>
                <div className="text-center">
                  <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest">Today's Spent</p>
                  <p className="text-xl font-black text-rose-700">Rs {summary.todayExpenses.toFixed(2)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <div className="space-y-3">
                {transactions.filter(t => t.date === todayStr).map(t => (
                  <div key={t.id} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                    <div className="flex items-center gap-3">
                      <div className={`${CATEGORY_CONFIG[t.category].color} p-2 rounded-lg text-white scale-75`}>
                        {CATEGORY_CONFIG[t.category].icon}
                      </div>
                      <div>
                        <span className="text-sm font-bold text-slate-700 block">{t.category}</span>
                        <span className="text-[10px] text-slate-400">{t.note}</span>
                      </div>
                    </div>
                ...
                {transactions.filter(t => t.date === todayStr).length === 0 && (
                  <p className="text-sm text-slate-400 italic py-4 text-center">No transactions entered for today yet.</p>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="lg:col-span-7">
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div className="max-h-[700px] overflow-y-auto scrollbar-hide">
                  {groupedTransactions.map(([date, items]) => {
                    ...
                    return (
                      <div key={date} className="border-b border-slate-100 last:border-0">
                        <div className="bg-slate-50 px-6 py-2 flex justify-between items-center">
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{date === todayStr ? 'Today' : date}</span>
                          <span className={`text-[10px] font-bold ${activeTab === 'income' ? 'text-emerald-600' : 'text-rose-500'}`}>
                            DAILY {activeTab === 'income' ? 'INCOME' : 'TOTAL'}: Rs {dailySum.toFixed(2)}
                          </span>
                        </div>
                        <div className="divide-y divide-slate-50">
                          {filteredItems.map((t) => (
                            <div key={t.id} className="p-4 hover:bg-slate-50/10 transition-colors group flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                <div className={`${CATEGORY_CONFIG[t.category].color} p-2.5 rounded-xl text-white shadow-sm`}>
                                  {CATEGORY_CONFIG[t.category].icon}
                                </div>
                                <div>
                                  <p className="font-semibold text-slate-800 leading-tight">{t.category}</p>
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <button onClick={() => removeTransaction(t.id)} className="opacity-0 group-hover:opacity-100 p-2 text-slate-300 hover:text-rose-500 rounded-lg transition-all">
                  ...
                  {transactions.filter(t => activeTab === 'income' ? t.type === TransactionType.INCOME : t.type === TransactionType.EXPENSE).length === 0 && (
                    <div className="p-20 text-center text-slate-400 italic">No records found. Start adding {activeTab === 'income' ? 'income' : 'expenses'}!</div>
                  )}
                </div>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-slate-200 px-6 py-3 flex items-center justify-around md:hidden z-50">
        <button onClick={() => setActiveTab('overview')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'overview' ? 'text-emerald-600' : 'text-slate-400'}`}>
          <LayoutDashboard className="w-6 h-6" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Dash</span>
        </button>
        <button onClick={() => setActiveTab('income')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'income' ? 'text-emerald-600' : 'text-slate-400'}`}>
          <TrendingUp className="w-6 h-6" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Income</span>
        </button>
        <div className="relative -mt-10">
          <button onClick={() => setActiveTab('manage')} className={`p-4 rounded-2xl shadow-xl shadow-emerald-200 transition-all ${activeTab === 'manage' ? 'bg-emerald-600 text-white scale-110' : 'bg-slate-200 text-slate-500'}`}>
